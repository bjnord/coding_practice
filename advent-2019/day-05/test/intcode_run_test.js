...
describe('intcode run tests', () => {
  ...
  it('should transform 1,0,0,0,99 to 2,0,0,0,99', () => {
    ...
    expect(intcode.run(program)).to.be.below(0);
  ...
  it('should leave 6,8,11,1,9,10,9,99,0,0,1,7 untransformed', () => {
    ...
    expect(intcode.run(programCopy)).to.be.below(0);
  ...
  it('should return n * 2 + 1 from headless mode', () => {
    ...
    expect(intcode.run(program, false, popValue, pushValue)).to.be.below(0);
  ...
  it('should support pause in headless mode when no input is available', () => {
    ...
    let pc = intcode.run(program, false, popValue, pushValue);
    ...
    // step 2 (halts)
    values.unshift(10);
    pc = intcode.run(program, false, popValue, pushValue, pc);
    expect(pc).to.be.below(0);
    expect(program[11]).to.eql(303);
