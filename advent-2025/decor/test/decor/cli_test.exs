defmodule Decor.CLITest do
  ...
  test "parts list" do
    ...
    |> Enum.each(fn [arg, exp_parts] ->
      {_, opts} = Decor.CLI.parse_args([arg, "bacon.txt"])
      assert opts[:parts] == exp_parts
    end)
  end

  test "verbose flag, default parts" do
    {_, opts} = Decor.CLI.parse_args(["--verbose", "eggs.txt"])
    assert opts[:verbose] == true
    assert opts[:parts] == [1, 2]
  end

  test "input file" do
    {input_file, _} = Decor.CLI.parse_args(["pancakes.txt"])
