...
their [joltage](../../advent-2020/day-10) rating,
a value from `1` to `9`. You make
...
joltage to overcome the
[static friction](https://en.wikipedia.org/wiki/Static_friction) of the system
and hits the big red "joltage limit safety override" button. You lose
count of the number of times she needs to confirm "yes, I'm sure" and
...
-   In the digit sequence `811111111111119`, the largest joltage can be
    found by turning on everything except some `1`s, producing
    `811111111119`.
-   In `234234234234278`, the largest joltage can be found by turning on
    everything except a `2` battery, a `3` battery, and another `2`
    battery near the start to produce `434234234278`.
...
I correctly guessed what part 2 was going to do, and the design I picked for part 1 was easily adapted for it. Elixir pattern matching and recursion is a beautiful thing!

But I found a bug in the back-tracking when finding more than 2 digits. I was diagnosing it when suddenly I realized an optimization: There is no point in ever trying a battery for a given digit, if there aren't enough batteries to the right of it for the remaining digits. After adding this, the bug never triggers... lucky twice.
