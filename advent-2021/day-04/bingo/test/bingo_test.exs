defmodule BingoTest do
  use ExUnit.Case
  doctest Bingo

  alias Bingo.Board, as: Board

  describe "puzzle example" do
    setup do
      [
        balls: [7, 4, 9, 5, 11, 17, 23, 2, 0, 14, 21, 24, 10, 16, 13, 6, 15, 25, 12, 22, 18, 20, 8, 19, 3, 26, 1],
        boards: [
          %Board{
            dim: 5,
            squares: [22, 13, 17, 11, 0, 8, 2, 23, 4, 24, 21, 9, 14, 16, 7, 6, 10, 3, 18, 5, 1, 12, 20, 15, 19],
            marks: 0b0,
            score: nil,
          },
          %Board{
            dim: 5,
            squares: [3, 15, 0, 2, 22, 9, 18, 13, 17, 5, 19, 8, 7, 25, 23, 20, 11, 10, 24, 4, 14, 21, 16, 12, 6],
            marks: 0b0,
            score: nil,
          },
          %Board{
            dim: 5,
            squares: [14, 21, 17, 24, 4, 10, 16, 15, 9, 19, 18, 8, 23, 26, 20, 22, 11, 13, 6, 5, 2, 0, 12, 3, 7],
            marks: 0b0,
            score: nil,
          },
        ],
      ]
    end

    test "first winning board finder", fixture do
      exp_winner = Enum.at(fixture.boards, 2)
      act_winner = Bingo.find_first_winner(fixture.boards, fixture.balls)
      assert act_winner.squares == exp_winner.squares
      assert act_winner.score == 4512
    end

    test "last winning board finder", fixture do
      exp_winner = Enum.at(fixture.boards, 1)
      act_winner = Bingo.find_last_winner(fixture.boards, fixture.balls)
      assert act_winner.squares == exp_winner.squares
      assert act_winner.score == 1924
    end
  end
end
